...
""")
	...
	def testDatalog(self, absoluteFilenameIn,nombreLignesIn):
		...
		for i in range(0,nombreLignesIn): # défile n lignes	
			valeur=random.randint(0,256) # génére une valeur aléatoire entre 0 et 1023
			myFile.write(str(i+1)+";"+str(int(time.time()))+";"+str(512+valeur)+";\n") # écrit les données dans le fichier	+ saut ligne 	
...
window.onload = function () { // au chargement de la page
	...
	if (canvas.getContext){
		...
		// récupère l'objet context du canvas pour dessin 
...
function drawData(stringDataIn) {
	...
	textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1); // se place à la fin -1 pour avant saut de ligne
	 ...
	 if (lines[debutData-1].match(/graphdata/)) { // graphdata se trouve à la xème ligne... avant, c'est des données... 
		...
		if (contextCanvas!=null) {
			...
			for (var i=debutData; i<lines.length-2; i++) { // défile les i
		
				textarea.value=textarea.value+String(i-debutData)+\"\\n\"; // ajoute la chaine au début - décale vers le bas...
				textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1) ; // se place à la fin -1 pour avant saut de ligne
				
				// -- coordonnees x,y courantes
				//- x 
				var nombreValeurs=lines.length-debutData+1; // nombre de valeurs à afficher
				
				if (nombreValeurs>canvas.width) { // adapte largeur par défaut canvas
					
					canvas.width=nombreValeurs;
					
					// carre  de la taille du canvas
					contextCanvas.fillStyle = \"rgb(255,255,200)\"; // fond jaune clair 
					contextCanvas.fillRect (0, 0, canvas.width, canvas.height); 
					contextCanvas.strokeStyle = \"rgb(0,0,255)\";
					
				} // fin if depasse largeur canvas
				
				x=i;
				
				//- y 
				var values=lines[i].split(\";\"); // récupère les 3 valeurs reçues par ligne
				
				textarea.value=textarea.value+values[2]+\"\\n\"; // ajoute la chaine au début - décale vers le bas...
				textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1) ; // se place à la fin -1 pour avant saut de ligne
				
				y=Number(values[2]); // récupère la valeur chiffrée à partir chaine reçue    
				y=y/1023*(canvas.height-1); // y mappée sur la hauteur canvas
	...
	} // fin if données graphiques reçues 
