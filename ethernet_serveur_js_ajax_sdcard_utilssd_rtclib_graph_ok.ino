...
#include <RTClibTimerEthernet.h> // lib perso pour implémentation timer avec RTCLib
...
#include <UtilsRTClibTimerEthernet.h> // inclusion de lib avec fonction analyse chaine sur port série pour RTClibTimer
    ...
    if (chaineRecue.startsWith("GET /&")) {
     ...
     //=========================================================================================
       ...
       param=utils.testInstructionString(chaineAnalyse, "testdatalog(",  true); // extrait le paramètre de la chaine avec debug
        ...
        cheminFichier.toCharArray(cheminFichierChar,param.length()+1); // récupère le param dans le tableau de char  
        //Serial.println(paramChar); // debug
        //getContentDirSerial(paramChar,0); // appelle la fonction listing contenu - passe le String en tableau de Char
        ...
        if (SD.exists(cheminFichierChar)) { // si le fichier existe 
              ...
              if (dataFile){ // le fichier est True si créé
                ...
                client.println(F("Ouverture fichier OK")); 
                 ...
                 dataFile.println("graphdata;"); // ajoute la ligne graphData - sera utilisé par le javascript pour savoir que fichier de données... 
     ...
     //--- la réponse HTML à afficher dans le navigateur
         ...
         client.println(F("<head>"));
           ...
           //----------------- Fonction principale éxécutée au chargement de la page ----------------
             ...
             client.println(F("window.onload = function () { // au chargement de la page"));
               ...
               //---- canvas ----
                ...
                client.println(F("canvas.width = 1000;"));
           ...
           //----------------------------------
            ...
            client.println(F("function drawData(stringDataIn) { "));
              ...
              //--- on commence par analyser la chaine reçue pour savoir si données à tracer... 
               ...
               client.println(F("var lines=stringDataIn.split(\"\\n\");"));
               client.println(F(""));

              //--- affiche messages dans textedit 
                 client.println(F("textarea.value=textarea.value+String(lines.length)+\"\\n\";")); // ajoute la chaine au début - décale vers le bas...
                 client.println(F("textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1) ;")); // se place à la fin -1 pour avant saut de ligne
 
              //--- test état lines[4] --- 
               client.println(F("if (lines[4].match(/graphdata/)) {")); // graphdata se trouve à la 5ème ligne... avant, c'est des données... 
                //--- affiche messages dans textedit 
                 client.println(F("textarea.value=textarea.value+\"Donnees graphiques recues\"+\"\\n\";")); // ajoute la chaine au début - décale vers le bas...
                 client.println(F("textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1) ;")); // se place à la fin -1 pour avant saut de ligne

             //--- on trace ensuite la courbe avec les données reçues 
              
               client.println(F("if (contextCanvas!=null) {"));
               
               
                 client.println(F("for (var i=5; i<lines.length-1; i++) {")); // défile les i

                    //--- affiche messages dans textedit 
                   client.println(F("textarea.value=textarea.value+String(i-5)+\"\\n\";")); // ajoute la chaine au début - décale vers le bas...
                   client.println(F("textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1) ;")); // se place à la fin -1 pour avant saut de ligne
                 
                   // -- coordonnees x,y courantes
                   //- x 

                   client.println(F("var nombreValeurs=lines.length-6;")); // nombre de valeurs à afficher
                   
                   client.println(F("if (nombreValeurs>canvas.width) {")); // adapte largeur par défaut canvas
                     client.println(F("canvas.width=nombreValeurs;")); // adapte largeur par défaut canvas
                      // carre  de la taille du canvas 
                      client.println(F("contextCanvas.fillStyle = \"rgb(255,255,200)\";")); // fond jaune clair 
                      client.println(F("contextCanvas.fillRect (0, 0, canvas.width, canvas.height);"));  
                      client.println(F("contextCanvas.strokeStyle = \"rgb(0,0,255)\";")); // dessin bleu 
                   client.println(F("}")); // fin if 
                   
                   client.println(F("x=i;")); // valeur de x
                   
                   //client.println(F("if (x>canvas.width)x=0;"));
                 
                   //- y 
                   client.println(F("var values=lines[i].split(\";\");")); // récupère les 3 valeurs reçues par ligne

                    //--- affiche messages dans textedit 
                   //client.println(F("textarea.value=textarea.value+values[2]+\"\\n\";")); // ajoute la chaine au début - décale vers le bas...
                   //client.println(F("textarea.setSelectionRange(textarea.selectionEnd-1,textarea.selectionEnd-1) ;")); // se place à la fin -1 pour avant saut de ligne

                   
                   client.println(F("y=Number(values[2]);")); // récupère la valeur chiffrée à partir chaine reçue                   
                 ...
                 client.println(F("else {"));
                  ...
                  client.println(F("textInputY.value=Number(stringDataIn);"));
            ...
            client.println(F("} // fin if ")); // fin if données graphiques reçues 
          ...
          //------- fin head = fin entete de la page HTML ----------          
           ...
           client.println(F("Valeur (0-1023) =<input type=\"text\" id=\"valeurY\" />"));
           client.println(F("<br/>"));
       ...
       if (SD.exists(nomFichierChar)) { // si le fichier existe 
         ...
         //---------- écriture des données dans le fichier ----------- 
              ...
              if (dataFile){ // le fichier est True si créé
                ...
                if (timerRTCEthernet[indexIn].compt==1) { // si début du fichier - le compteur déjà incrémenté quand fonction appelée 

                 dataFile.println("graphdata;"); // ajoute la ligne graphData - sera utilisé par le javascript pour savoir que fichier de données... 
                  
                } // fin if première ligne 
